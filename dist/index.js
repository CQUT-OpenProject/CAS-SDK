import { c as isCasErrorOfKind, o as CasError, r as getSecretParam, s as isCasError } from "./encryptor-CTVlEYoC.js";
...
const defaultFetcher = async (request) => {
	if (typeof globalThis.fetch !== "function") throw new CasError("CONFIGURATION_ERROR", "globalThis.fetch is required; provide a server-side Fetcher");
	const init = {
...
function abortError(signal, step) {
	return new CasError(signal.reason?.name === "TimeoutError" ? "TIMEOUT" : "ABORTED", `${step}: request cancelled`, {
		step,
		cause: signal.reason
	});
}
function deadline(signal, timeoutMs = 3e4) {
	if (!Number.isInteger(timeoutMs) || timeoutMs <= 0 || timeoutMs > 2147483647) throw new CasError("CONFIGURATION_ERROR", "timeoutMs must be a positive 32-bit integer");
...
async function readResponse(response, signal, step, kind) {
	...
	try {
		while (true) {
			...
			if (size > 65536) throw new CasError(kind, `${step}: response exceeds 64 KiB`, {
				step,
		...
		throw new CasError("NETWORK_ERROR", `${step}: response read failed`, {
			step,
			cause
		});
	} finally {
...
var CasClient = class {
		...
		for (let redirects = 0;; redirects++) {
			...
			for (let attempt = 0; attempt < 2; attempt++) {
				try {
					...
					if (attempt === 1 || !(error instanceof CasError) || error.kind !== "NETWORK_ERROR") throw error;
				}
			...
			if (!REDIRECTS.has(res.status)) {
				if (res.status !== 200) throw new CasError("PROTOCOL_ERROR", "fetchLoginPage: unexpected HTTP status", {
					step,
					status: res.status
				});
				return {
			...
			if (redirects === 5) throw new CasError("PROTOCOL_ERROR", "fetchLoginPage: too many redirects", { step });
			...
			try {
				...
				throw new CasError("PROTOCOL_ERROR", "fetchLoginPage: invalid redirect", {
					step,
					cause
				});
			}
	...
	async doLogin(credentials, refererUrl, options) {
		...
		if (!data || typeof data !== "object" || !("code" in data) || !["number", "string"].includes(typeof data.code) || !Number.isFinite(Number(data.code))) throw new CasError("PROTOCOL_ERROR", "doLogin: missing response code", {
			step,
			status: res.status
		});
		if (res.status >= 300 && res.status < 400) throw new CasError("PROTOCOL_ERROR", "doLogin: unexpected redirect", {
			step,
			status: res.status
		});
		if (res.status >= 400 || Number(data.code) !== 200) {
			const captcha = "msg" in data && typeof data.msg === "string" && /验证码|captcha/i.test(data.msg);
			throw new CasError(captcha ? "CAPTCHA_REQUIRED" : "AUTH_FAILED", captcha ? "Captcha required" : "Campus credentials rejected", {
				step,
				status: res.status
			});
		}
	...
	async acquireServiceTicket(casLoginUrl, serviceWithClientId, refererUrl, options) {
		...
		if (!isServiceTicket(ticket)) throw new CasError("PROTOCOL_ERROR", "acquireServiceTicket: ticket was not issued", {
			step,
			status: res.status
		});
		return ticket;
	...
	async safeLogin(options) {
		try {
			...
			if (error instanceof CasError) return {
				ok: false,
				error
			};
			throw error;
	...
	async request(req, jar, step) {
		...
		try {
			...
			if (req.signal.aborted) throw abortError(req.signal, step);
			if (cause instanceof CasError) throw cause;
			throw new CasError("NETWORK_ERROR", `${step}: request failed`, {
				step,
				cause
			});
		}
...
//#endregion
export { CasClient, CasError, MemoryCookieJar, assertServiceTicket, createCasClient, defaultFetcher, isCasError, isCasErrorOfKind, isServiceTicket, parseCasValidationResponse };
